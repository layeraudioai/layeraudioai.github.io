...
class LayAI {
    constructor() {
        ...
        const isSecure = hostname.includes('https://');
        if (isProduction && !isLocalDev) {
            location.replace('https://' + location.host + location.pathname + location.search + location.hash);
        }
        if (!isSecure && isProduction && !isLocalDev)
        {
            this.addLog("Connection not secure","warning");
            location.replace("https://layai.ca/");
        }
    }
    ...
    async getSample(buffer, view, numFrames, numChannels, offset) {
        ...
        for (let i = 0; i < numFrames; i++) {
            for (let channel = 0; channel < numChannels; channel++) {
                ...
                const sample = buffer.getChannelData(channel)[(i+(i%this.sampleValue))%((frameIndex)%numFrames)];
                const clamped = Math.max(-1, Math.min(1, sample));
                view.setInt16(off, clamped < 0 ? clamped * 0x8000 : clamped * 0x7fff, true);
                off += this.bytespersample;
            }
